...
void SER_init (void)
{
	...
	LPC_UART->DLL = 12;                     /* 38400 Baud Rate @ 12.0 MHZ PCLK */
	
	LPC_UART->FDR = 0x85;                   /* FR 1.627, DIVADDVAL 5, MULVAL 8  */
	LPC_UART->DLM = 0;                      /* High divisor latch = 0           */
	LPC_UART->LCR = 0x03;                   /* DLAB = 0                         */
	LPC_UART->IER = 0x1; 						 /* enable UART int						 */
	
	LPC_UART->FCR=0x01; 							//enable FIFO 1 byte
//	LPC_UART->MCR=0x40; 							//enable RTS Flow Control
	
	
	//must be set BEFORE enabling the IRQ
